...
resource "aws_iam_role" "ecs_task_execution_role" {
  name = "token-pilot-ecs-execution-role"
...
resource "aws_cloudwatch_log_group" "ecs_log_group" {
  name              = "/ecs/token-pilot-app"
  retention_in_days = 7
}

# 3. 선수 명단 (Task Definition): 스프링 부트 + 그라파나
resource "aws_ecs_task_definition" "app_task" {
  family                   = "token-pilot-app-task"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    # 🏃‍♂️ 1번 선수: 스프링 부트
    {
      name         = "token-pilot-container"
      image        = "${aws_ecr_repository.token_pilot_repo.repository_url}:latest"
      essential    = true
    ...
    # 🎨 2번 선수: 그라파나
    {
      name  = "token-pilot-grafana-container"
      image = "${aws_ecr_repository.token_pilot_grafana_repo.repository_url}:latest"
    ...
    {
      name         = "token-pilot-prometheus-container"
      image        = "${aws_ecr_repository.token_pilot_prometheus_repo.repository_url}:latest"
      essential    = true
...
resource "aws_ecs_service" "app_service" {
  name            = "token-pilot-test-app-task-service"
  cluster         = aws_ecs_cluster.token_pilot_cluster.id
  ...
  load_balancer {
    ...
    container_name   = "token-pilot-container" # 컨테이너 이름과 똑같아야 함
    container_port   = 8080
  }

  load_balancer {
    target_group_arn = aws_lb_target_group.grafana_tg.arn
    container_name   = "token-pilot-grafana-container" # 컨테이너 이름과 똑같아야 함
    container_port   = 3000
  }
